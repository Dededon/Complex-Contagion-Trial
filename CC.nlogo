...
to setup
  ...
  ask turtles [ set color black set cango? false ]
  set saturated? false
  if count turtles > 0 [
    ask one-of turtles [
...
to restart
  ...
  set-current-plot "Saturated Ratio"
  clear-plot
  reset-ticks
end
...
to run-step
  ...
  if count turtles with [ color = black and cango? = false ] = 0 [ set saturated? true stop ]
  ...
  if not saturated? [
    ask one-of turtles with [color = black ] [
      ...
      ifelse count-triggered >= threshold [
        set color red ask turtles with [ self != myself ] [ set cango? false ]
      ]
      [
        set cango? true
      ]
...
"" ""
PENS
"% Saturated" 1.0 0 -16777216 true "" "plot percent-saturated"
